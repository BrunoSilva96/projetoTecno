...
export const userService = {
   create: async (name: string, email: string, password: string, roleId: number ) => {
   ...
   updateUser: async (  
      ...
      const updatedUsers = await Users.update({name: name}, {where: {email}})

      return updatedUsers;
   },

   updateRole: async (roleId: number, email: string) => {
      const updateRole = await Users.update({roleId: roleId}, {where: {email}})
